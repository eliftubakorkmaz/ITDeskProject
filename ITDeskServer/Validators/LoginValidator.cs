...
public sealed class LoginValidator : AbstractValidator<LoginDto>
{
    ...
    {
        RuleFor(p => p.UserNameOrEmail).NotEmpty().WithMessage("Geçerli bir kullanıcı adı yada mail adresi girin.");
        RuleFor(p => p.UserNameOrEmail).MinimumLength(3).WithMessage("Geçerli bir kullanıcı adı yada mail adresi girin.");
